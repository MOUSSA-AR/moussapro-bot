...
@catub.cat_cmd(
    ...
    info={
        "header": "للحصول على احدث اصدارات السحر",
        "usage": "{tr}magisk",
    },
)
async def kakashi(event):
    ...
    releases = "**احدث اصدارات السحر**\n\n"
    for name, release_url in magisk_dict.items():
        data = get(release_url).json()
        releases += (
            f'{name}: [التطبيق v{data["magisk"]["version"]}]({data["magisk"]["link"]}) | '
...
async def device_info(event):
    ...
    if results:
        reply = f"**نتائج البحث عن {codename}**:\n\n"
        for item in results:
            reply += (
                f"**الشركة المصنعة**: {item['brand']}\n"
                f"**الاسم**: {item['name']}\n"
                f"**النموزج**: {item['model']}\n\n"
            )
    else:
...
async def codename_info(event):
    ...
    if brand and device:
        ...
        return await edit_delete(event, "`الاستخدام: .كود <الشركة المصنعة> <الجهاز>`")
    ...
    if results:
        reply = f"**نتائج البحث عن {brand} {device}**:\n\n"
        if len(results) > 8:
            results = results[:8]
        for item in results:
            reply += (
        ...
        reply = f"`لا يمكنني العثور على {device} الجهاز من الكود!`\n"
    await edit_or_reply(event, reply)
...
async def devices_specifications(event):
    ...
    if brand and device:
        ...
        return await edit_delete(event, "`الاستخدام: .حول <الشركة المصنعة> <الجهاز>`")
    all_brands = (
        BeautifulSoup(
    ...
    try:
        ...
        return await edit_delete(event, f"`{brand} علامة تجارية غير معروفة!`")
...
async def twrp(event):
    ...
    if device:
        ...
        return await edit_delete(event, "`الاستخدام: .موقع <الكود>`")
    url = get(f"https://dl.twrp.me/{device}/")
    if url.status_code == 404:
        reply = f"`تعذر العثور على تنزيلات الموقع ل {device}!`\n"

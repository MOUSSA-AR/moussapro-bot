...
@moussabot.tgbot.on(CallbackQuery(data=re.compile(r"^age_verification_true")))
...
@moussabot.tgbot.on(CallbackQuery(data=re.compile(r"^age_verification_false")))
...
@moussabot.tgbot.on(CallbackQuery(data=re.compile(r"^chg_of_decision_")))
async def chg_of_decision_(event: CallbackQuery):
    ...
    try:
        await event.edit(
            ...
            file="https://telegra.ph/file/30a31c94e3b80c147bc15.jpg",
            buttons=buttons,
        )
    except MessageNotModifiedError:
        pass
